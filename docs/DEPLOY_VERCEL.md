...
5. **Environment Variables** (Production **et** Preview) :
   ...
   - **Workflow voyageur (PRD v3)** : `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `NEXT_PUBLIC_DA_CONTACT_EMAIL`, `NEXT_PUBLIC_WHATSAPP_DA_NUMBER` (indicatif pays + numéro, chiffres uniquement pour le lien WhatsApp). Sans Resend, le lancement de workflow reste possible (qualification + journal d’activité), sans email automatique.
